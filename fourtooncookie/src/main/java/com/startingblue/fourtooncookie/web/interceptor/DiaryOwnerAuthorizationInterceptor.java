...
public class DiaryOwnerAuthorizationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (!isValidMemberId(memberId, response)) {
            return false;
        }

        String diaryId = extractPathVariable(request.getRequestURI());
        if (!isValidDiaryId(diaryId, response)) {
            return false;
        }

        try {
            return handleAuthorization(UUID.fromString(memberId), Long.parseLong(Objects.requireNonNull(diaryId)), response);
